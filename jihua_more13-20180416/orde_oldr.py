...
headers3 = dict()
headers3['user-agent'] = 'Mozilla/5.0 (Windows NT 6.1; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.62 Safari/537.36'
...
def take_action():	
	...
	if r.json()['success']:
										...
										if r.ok:
											...
											if True:#p:
												...
												if p:
													...
													print(issue)
													bets = [{"name":int(x),"room_id":"0","lottery_id":"54","issue":issue,"method_id":"700005","bet_num":int(x),"odds":str(BIG_PEILV),"bet_money":touzhu,"$$hashKey":"object:%d" % (300+i)} for (i,x) in enumerate(b[3:8])] 
		...
		payload = {
			"user_name":"sanfen123za","password":"word12531","url":quote('手机客户端')
		}
